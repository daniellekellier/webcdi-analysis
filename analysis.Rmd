...
a <- list(filter(slim_webcdi_data, study_name == 'Facebook-2-WG2'), slim_wordbank_data)

for (i in 1:length((a))) {
  temp <- a[[i]]
  temp <- temp[,c("source","id","form","study_group","study_name","age","sex","mom_ed","production","comprehension","n")]
  a[[i]] <- temp
  comp_preds
...
levels(both_data$mom_ed)<-c("Below Secondary","Secondary","College and Above")
...
prop_prod <- both_data %>% 
  group_by(age) %>% 
  filter(study_group %in% c("MTurk-3","MTurk-4","MTurk-5", "MTurk-7") | 
           source == "wordbank") %>% 
